...
struct index_entry *qkc_load_index_entry(struct qkc_database *database) {
...
bool qkc_write_index_entry(struct qkc_database *database, const float data_size) {
	struct index_entry *recent_entry = qkc_load_index_entry(database);
	...
	printf("%f end: %f \n", new_entry.start_bytes, new_entry.end_bytes);
